...
getMessage :: (MonadGet m, MonadQuota m) => m Message
getMessage = do
    numSegments <- get32
    segmentSizes <- UV.replicateM numSegments get32
    -- pad up to the next word boundary:
    when ((1 + UV.length segmentSizes) `mod` 2 /= 0) get32
    UV.mapM getSegment segmentSizes
  where
    getLimited size getter = invoice size >> getter
    get32 = getLimited 4 getWord32le
    get64 = getLimited 8 getWord64le
    getSegment size = UV.replicateM size get64

-- | Write out a Cap'n Proto message.
putMessage :: (MonadPut m) => Message -> m ()
putMessage msg = do
    putWord32le (BV.length msg)
    BV.mapM_ (putWord32le . UV.length) msg
