...
enumInstances thisMod name =
  let rawName = gName (rawModule thisMod) name
   in instance_
        []
        ["Classes"]
        "Decerialize"
...
dataToDecls thisMod data_@P.Data {firstClass} =
  concat $
...
typeParamsDecerializeCtx typeParams =
  let vars = typeParamVars typeParams
...
fieldsToDecerialize thisMod typeName firstClass fields =
  let fieldGetter parentName name =
        egName
          (rawModule thisMod)
          (Name.unQToLocal $ Name.getterName $ Name.mkSub parentName name)
   ...
   in case fields of
        ...
        _ ->
          EFApp
            (ELName variantName)
            [ let getter = EApp (fieldGetter typeName name) [euName "raw"]
               in if name == "union'"
                    then -- unions decerialize from the same type as their parents. Don't
                    -- do anything but pass it off.
                      EApp (egName ["Classes"] "decerialize") [euName "raw"]
                    else
                      if cerialEq type_
                        then getter
                        else EBind getter (egName ["Classes"] "decerialize")
  ...
  let unknownCtor = Name.mkSub cerialName "unknown'"
   in ECase (euName "value_") $
        [ let setter = Name.unQToLocal $ Name.setterName variantName
          ...
          ++ let setter = Name.unQToLocal $ Name.setterName unknownCtor
              in [ ( PLCtor unknownCtor [PVar "tag"],
...
fieldsToMarshal thisMod typeName firstClass fields =
  let variantName = productVariantName typeName firstClass
   in ECase
        (euName "value_")
        [ ( case fields of
...
firstClassInstances _thisMod P.Data {typeName, typeParams} =
  let typ = typeWithParams typeName typeParams
   ...
   in [ instance_ ctx ["Classes"] "Cerialize" [tuName "s", typ] [],
        instance_
          ctx
          ["Classes"]
          "Cerialize"
          [tuName "s", TApp (tgName ["V"] "Vector") [typ]]
...
ifaceClassDecl thisMod P.IFace {name = Name.CapnpQ {local = name}, methods, supers, typeParams} =
  let typeParamNames = map (Name.typeVarName . C.paramName) typeParams
   in DcClass
        { ctx =
            let superConstraints =
             ...
             in TApp (tgName ["MonadIO"] "MonadIO") [tuName "m"]
          ...
          decls =
            let mkName = mkMethodName name
             in CdMinimal [mkName name | P.Method {name} <- methods]
                  : concat
                    [ [ CdValueDecl
                          (mkName methodName)
                          ( TFn
...
ifaceExportFn thisMod iface@P.IFace {name = Name.CapnpQ {local}, typeParams, ancestors} =
  let typeParamVars = map (TVar . Name.typeVarName . C.paramName) typeParams
   in DcValue
        { typ =
            TCtx
                                            ...
                                            ECase (euName "methodId_") $
                                              [ ( PInt i,
                                                  EApp
                                                    ...
                                                    [ let methodName = mkMethodName ifaceName mname
                                                       in EApp
                                                            ( if thisMod == ifaceMod
...
ifaceInstances thisMod iface@P.IFace {name = Name.CapnpQ {local = name}, typeParams} =
  let typ = typeWithParams name (map C.paramName typeParams)
   in [ instance_
          []
          ["Rpc"]
          "IsClient"
  ...
  where
    ...
    go (P.IFace {name = Name.CapnpQ {local, fileId}, interfaceId, methods}, C.ListBrand types) =
      let className = Name.mkSub local "server_"
       ...
       in -- Can't use 'instance_' here, because we don't know ahead of time what
          -- module the class is in.
          DcInstance
            { ctx = [],
              typ = TApp classType $ [tStd_ "IO", typ] ++ classArgs,
              defs =
                [ let methodName = mkMethodName local mname
                   in iValue methodName [PLCtor name [PVar "client"]] $
                        EApp
...
marshalField MarshalField {thisMod, into, localQField, from, type_, inUnion} =
  let setter = egName (rawModule thisMod) $ Name.unQToLocal (Name.setterName localQField)
   ...
   in case type_ of
        ...
        C.CompositeType _ ->
          EDo
            ( if Name.getUnQ localQField == "union'"
                then -- Anonymous union; has the same Cerial as us. Just delegate.
                  []
                else
                  if inUnion
                    then [DoBind into $ EApp setter [euName into]]
                    else [DoBind into $ EApp getter [euName into]]
            )
...
typeToType thisMod (C.WordType (C.EnumType Name.CapnpQ {local, fileId})) =
  ...
  if thisMod == fileId
    then tgName (rawModule fileId) local
