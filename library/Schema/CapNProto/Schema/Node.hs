...
displayName :: DS.Field S.Node (Maybe Text)
displayName = DS.PtrField 0

displayNamePrefixLength :: DS.Field S.Node Word32
displayNamePrefixLength = DS.fromBits 64 96

scopeId :: DS.Field S.Node S.Id
scopeId = DS.fromBits 128 192

parameters :: DS.Field S.Node (Maybe (DS.List Parameter))
parameters = DS.PtrField 5

isGeneric :: DS.Field S.Node Bool
isGeneric = DS.fromBits 288 289

nestedNodes :: DS.Field S.Node (Maybe (DS.List NestedNode))
nestedNodes = DS.PtrField 1

annotations :: DS.Field S.Node (Maybe (DS.List S.Annotation))
