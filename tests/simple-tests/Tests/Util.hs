...
capnpEncode msgValue meta = do
  (exitStatus, contents) <- runResourceT $ do
    (hin, hout, phandle) <- interactCapnpWithSchema "encode" (msgSchema meta) [msgType meta]
    contents <- lift $ do
      forkIO $ do
...
capnpDecode encodedMsg meta = do
  (exitStatus, contents) <- runResourceT $ do
    (hin, hout, phandle) <- interactCapnpWithSchema "decode" (msgSchema meta) [msgType meta]
    contents <- lift $ do
      forkIO $ do
...
capnpCompile msgSchema outputArg = do
  (exitStatus, contents) <- runResourceT $ do
    (hin, hout, phandle) <- interactCapnpWithSchema "compile" msgSchema [outputArg]
    contents <- lift $ do
      hSetBinaryMode hout True
      BS.hGetContents hout
    exitStatus <- lift $ waitForProcess phandle
...
capnpId = do
  (exitStatus, cid) <- runResourceT $ do
...
interactCapnpWithSchema :: String -> String -> [String] -> ResourceT IO (Handle, Handle, ProcessHandle)
